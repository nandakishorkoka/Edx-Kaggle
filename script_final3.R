...
summary(model) 
model$importance
model <- randomForest(Popular ~ . UniqueID, data = train , importance=TRUE, ntree=100)
model <- randomForest(Popular ~ . - UniqueID, data = train , importance=TRUE, ntree=100)
pred_labels = predict(model, newdata=test, type="prob") 
pred = prediction(pred_labels[,2], test$Popular) 
auc.tmp <- performance(pred,"auc")
auc <- as.numeric(auc.tmp@y.values)
print (paste("AUC: ", auc, sep=" ")) 
model <- randomForest(Popular ~ . - UniqueID - pred_labels = predict(model, newdata=test, type="prob") , data = train , importance=TRUE, ntree=100)
pred = prediction(pred_labels[,2], test$Popular) 
auc.tmp <- performance(pred,"auc")
auc <- as.numeric(auc.tmp@y.values)
print (paste("AUC: ", auc, sep=" ")) 
model <- randomForest(Popular ~ . - UniqueID - Hour, pred_labels = predict(model, newdata=test, type="prob") , data = train , importance=TRUE, ntree=100)
pred_labels = predict(model, newdata=test, type="prob") 
pred = prediction(pred_labels[,2], test$Popular) 
auc.tmp <- performance(pred,"auc")
auc <- as.numeric(auc.tmp@y.values)
...
model$importance 
print (paste("AUC: ", auc, sep=" ")) 
model <- randomForest(Popular ~ . - UniqueID , pred_labels = predict(model, newdata=test, type="prob") , data = train , importance=TRUE, ntree=500)
pred_labels = predict(model, newdata=test, type="prob") 
pred = prediction(pred_labels[,2], test$Popular) 
auc.tmp <- performance(pred,"auc")
auc <- as.numeric(auc.tmp@y.values)
print (paste("AUC: ", auc, sep=" "))
save(model, file="rf_model.Rdata")
savehistory(file="script_final3.R") 
